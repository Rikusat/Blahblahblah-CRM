...
""", unsafe_allow_html=True)
    ...
    SIZE_MAP = {"小": "1rem", "中": "1.3rem", "大": "1.7rem", "特大": "2.2rem"}
    ...
    def _render_board(board_type: str, label: str, editable: bool):
        ...
        color   = _safe_color(data.get("color", ""), "#FF8C00" if board_type == "admin" else "#e0e0e0")
        size    = data.get("size", "中") if data.get("size", "中") in SIZE_MAP else "中"

        st.markdown(
            f"<div style='font-size:.6rem;color:#555;font-family:monospace;letter-spacing:3px;"
            f"text-transform:uppercase;margin-bottom:.4rem;'>{label}</div>",
            unsafe_allow_html=True,
        )
        msg = content if content else "— メッセージなし —"
        msg_color = color if content else "#3a3a3a"
        st.markdown(
            f"<div style='color:{msg_color};font-size:{SIZE_MAP[size]};font-family:monospace;"
            f"line-height:1.8;white-space:pre-wrap;border-left:3px solid {'#FF8C00' if board_type=='admin' else '#444'};"
            f"padding:.6rem 1rem;margin-bottom:.5rem;'>{msg}</div>",
            unsafe_allow_html=True,
        )
        if editable:
            with st.expander("✏️ 編集"):
                draft = st.text_area("メッセージ", value=content, key=f"board_{board_type}_txt", height=100, label_visibility="collapsed", placeholder="メッセージを入力...")
    ...
    tab_admin, tab_sales = st.tabs(["📌 管理者掲示板", "📋 営業掲示板"])
    with tab_admin:
